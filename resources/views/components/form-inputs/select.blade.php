    ...
    <div class="input-group input-icon right in-grp1">
      ...
      <select id="{{$name}}" name="{{$name}}" class="form-control1" @if(isset($disabled)) disabled @endif>
        @foreach($options as $option)
          <option value="{{$option['value']}}" @if(old('type')==$option['value']) selected @elseif(isset($option['selected'])) @if($option['selected'] == $option['value']) selected @endif @endif>{{$option['name']}}</option>
        @endforeach
      </select>
    </div>
  </div>
